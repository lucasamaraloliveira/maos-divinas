...
const Services: React.FC = () => {
  return (
    <section className="py-20 md:py-32 bg-[#faf7f7]" id="servicos">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16 md:mb-24 opacity-0 animate-reveal-up">
          <h3 className="text-[#ed4b82] uppercase tracking-[0.3em] font-semibold text-xs md:text-sm mb-4">Experiência</h3>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif text-[#2c2c2c]">Nossos Serviços</h2>
          <div className="w-16 md:w-24 h-px bg-[#ed4b82] mx-auto mt-6 transition-all duration-700 hover:w-40"></div>
        </div>
          ...
          {services.map((service, index) => (
            ...
            >
              <div className="h-56 md:h-64 overflow-hidden relative">
                ...
                />
                <div className="absolute top-4 right-4 bg-white/95 backdrop-blur px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-[#ed4b82] translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 shadow-sm">
                  Premium
                </div>
              </div>
              <div className="p-6 md:p-8">
                <h4 className="text-xl md:text-2xl font-serif mb-2 text-[#333333] group-hover:text-[#ed4b82] transition-colors">{service.title}</h4>
                <p className="text-[#ed4b82] text-xs font-bold mb-4 uppercase tracking-[0.2em]">{service.price}</p>
                ...
                <a href="#contato" className="relative inline-block text-[10px] uppercase tracking-[0.2em] font-bold pb-1 group/link transition-all">
                  ...
                  <div className="absolute bottom-0 left-0 w-full h-px bg-[#ed4b82] scale-x-50 group-hover/link:scale-x-100 transition-transform origin-left"></div>
                </a>
              </div>
            </div>
          ))}
