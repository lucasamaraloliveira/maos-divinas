    ...
    <>
      ...
      >
        <div className="flex items-center justify-between">
          <div className={`font-serif text-xl md:text-2xl font-bold tracking-[0.25em] transition-all duration-500 
            ${scrolled ? 'text-[#ed4b82] scale-95' : 'text-[#ed4b82] scale-100'}`}
          >
            MÃOS DIVINAS
          </div>
              ...
              <li key={link.name}>
                <a
                  ...
                  className={`text-[10px] uppercase tracking-[0.3em] font-bold transition-all duration-500 relative group
                    ${scrolled ? (activeSection === link.id ? 'text-[#ed4b82]' : 'text-gray-800') : (activeSection === link.id ? 'text-[#ed4b82]' : 'text-[#333333]')}`}
                >
                  {link.name}
                  <span className={`absolute -bottom-1 left-0 w-0 h-px bg-[#ed4b82] transition-all duration-500 group-hover:w-full ${activeSection === link.id ? 'w-full' : ''}`}></span>
                </a>
              </li>
            ))}
          </ul>
          ...
          <button
            onClick={toggleMenu}
            className={`lg:hidden p-2 transition-transform active:scale-90 ${scrolled ? 'text-gray-800' : 'text-[#2c2c2c]'}`}
            aria-label="Menu"
          >
      ...
      <div className={`fixed inset-0 z-[60] lg:hidden transition-all duration-700 ${isMenuOpen ? 'visible' : 'invisible'}`}>
        ...
        <div className={`absolute right-0 top-0 h-full w-[85%] max-w-sm bg-white p-12 flex flex-col shadow-2xl transition-transform duration-700 ease-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="font-serif text-2xl font-bold tracking-widest text-[#ed4b82] mb-16 opacity-0 animate-reveal-right delay-100">
            MÃOS DIVINAS
          </div>
              ...
              <li key={link.name} className={`opacity-0 ${isMenuOpen ? 'animate-reveal-right' : ''}`} style={{ animationDelay: `${200 + idx * 100}ms` }}>
                <a
                  ...
                  className={`text-sm uppercase tracking-[0.4em] block py-2 transition-all hover:translate-x-2
                    ${activeSection === link.id ? 'text-[#ed4b82] font-bold' : 'text-gray-500'}`}
                >
                  {link.name}
                </a>
              </li>
          ...
          <div className={`mt-auto opacity-0 ${isMenuOpen ? 'animate-reveal-up' : ''}`} style={{ animationDelay: '800ms' }}>
            <a
              href="#contato"
              onClick={toggleMenu}
              className="block w-full text-center py-5 bg-[#ed4b82] text-white uppercase tracking-widest text-[10px] font-bold shadow-2xl transition-all active:scale-95 hover:bg-[#d43d6f]"
            >
              Agendar Experiência
            </a>
          </div>
